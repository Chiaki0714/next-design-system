# Chiaki Design System Rulebook v3
...
**display / grid / flex / gap / align などの内部レイアウト責務は持たせない。**

Figma テンプレート上では `Container` に `flex-direction` や `gap` が内包されている場合がある。  
ただし、実装ではその構造をそのまま写さず、`Container` と `Inner` に責務分離してよい。

#### `inner`

`container` の内側で内部レイアウトを担当する層。  
`flex` / `grid` / `gap` / align / justify / columns などは `inner` 側に持たせる。

Figma の Dev Mode で `Container` に表示される `gap` や `align-items` は、コード側では原則 `Inner` に移す。
...
- 更新日: 2026-05-01
- Figma の `Container` 内包構造を、実装では `Section / Container / Inner` に責務分離する方針を明文化
- Navbar / Rich Text / LP Section の Dev Mode CSS をもとに wrapper 初期値を整理
- `Section / Container / Inner / Split / Grid / Stack` の Layout Component API を追加
- tablet 専用デザインがない場合は `mobile default + lg desktop` で扱う方針を追加

### DesignSystem v2

- 更新日: 2026-05-01
- Figma完全再現ではなく、Figma参照 + Next.jsテンプレ実装の方針を追加
- Dev Modeを自動実装ではなく値確認ツールとして使う方針を追加
- Tailwind / CSS Modules / shadcn の役割を再整理
- 案件用テンプレートとしての実装フローを追加
...
# Part 4. Figma Reference + Next.js Template Flow

## 1. 今後の制作方針

今後の制作では、Figma をそのままコード化することを目的にしない。  
Figma は **見た目・構成・余白・トーンを決める設計図** として扱い、Next.js 側では再利用可能なテンプレートコンポーネントへ落とし込む。

つまり、基本方針は次の通り。

```txt
Figma = デザイン判断の場所
Code  = 品質・保守性・レスポンシブ・SEOを担保する場所
```

Figma とコードの token 名や class 名を完全一致させる必要はない。  
ただし、概念は必ず揃える。

```txt
Figma: color / font / spacing / container / breakpoint
Code : theme / typography / section / container / component
```

## 2. Dev Mode の扱い

Dev Mode は自動実装ツールとして使わない。  
主な用途は以下に限定する。

- 使用色の確認
- font-size / line-height / weight の確認
- 余白・幅・高さの確認
- 画像・SVG・アイコンの書き出し
- breakpoint ごとの見え方確認
- component variant の意図確認

Dev Mode で出力される CSS をそのまま貼り付けるのは禁止。  
必要な値だけを確認し、既存の token / component / section template に変換する。

## 3. 命名方針

CSS custom properties は CSS 標準に合わせて kebab-case とする。
...
React props / TypeScript の定数 / 関数名は camelCase とする。
...
CSS と TypeScript の命名を無理に揃えない。  
それぞれの言語・役割に合った命名を優先する。

## 4. Tailwind の位置づけ

Tailwind は使用前提とするが、Tailwind だけですべてを完結させない。  
基本は次の使い分けとする。

- レイアウト・余白・レスポンシブ: Tailwind
- 複雑な装飾・局所的な表現: CSS Modules
- 全体 token: globals.css
- 再利用 UI: React Component
- 複雑な motion: client component に局所化

任意値は原則避けるが、Figma 上の特殊な表現を再現する場合は、理由が明確であれば CSS Modules 側に閉じ込めてよい。

## 5. shadcn の位置づけ

shadcn はアプリ UI のために全面採用するものではない。  
このテンプレートでは、次の用途に限定して参考にする。

- token 設計
- variant 設計
- `cn()` による className 合成
- button / badge / card など、Webサイトでも使いやすい最小 UI
- dialog / accordion など、必要になった場合の挙動付き UI

Webサイト制作では、section 単位の見せ方を優先する。  
shadcn コンポーネントを増やすこと自体を目的にしない。

## 6. 推奨コンポーネント構成

案件テンプレートでは、次の骨格を基本とする。
...
`layout` は構造、`typography` は文字、`ui` は小さな部品、`sections` は案件で組み替える大きなブロックとして扱う。

## 7. 実装フロー

### 1. Figma で方向性を決める

- どのテンプレート section に近いかを確認する
- 余白・文字サイズ・色のトーンを見る
- 特殊な表現が必要か確認する

### 2. コード側の section に当てはめる

Figma の見た目を、まず既存 section に寄せる。
...
既存 section で 80% 以上表現できる場合は、新規 component を作らない。
...
いきなり新規 component を作らない。

### 4. スマホで破綻しないか確認する

PC の見た目より、まずスマホで自然に読めることを優先する。  
PC の装飾は後から足す。

## 8. 案件用テンプレートとして守ること

- Figma を完璧に再現しようとしすぎない
- 実装で再利用できる形に翻訳する
- section の種類を増やしすぎない
- 1 案件だけのための特殊 component を増やさない
- 画像・余白・アニメーションはスマホ基準で確認する
- 公開後の修正がしやすい構造を優先する

短時間で安定した品質を出すために、毎回ゼロから作らず、既存 section を組み替える。


## 9. Figma Wrapper をコードへ翻訳するルール

### 基本方針

Figma の wrapper 構造と、コードの wrapper 構造は完全一致させなくてよい。  
Figma はデザイン作業のために `Container` へ `max-width / padding / flex / gap / align` をまとめていることがある。  
コードでは、保守性を優先して以下に分解する。

```txt
Section
= セクション全体の上下余白・背景・意味づけ

Container
= 横幅制御・左右 padding・中央寄せ

Inner
= 内部レイアウト・gap・align・justify・column / row
...
Dev Mode で次のように表示された場合、

```css
display: flex;
max-width: var(--max-width-max-w-7xl, 1280px);
...
<Section spacing="default">
  <Container size="7xl">
    <Inner direction="column" justify="center" align="start" gap="16">
      ...
    </Inner>
  </Container>
</Section>
...
| Figma Dev Mode | Code component |
|---|---|
| `padding: var(--section-padding-y) 0` | `Section spacing="default"` |
| `background: var(--base-background)` | `Section tone="default"` |
| `max-width: var(--max-width-max-w-7xl)` | `Container size="7xl"` |
| `padding: 0 var(--container-padding-x)` | `Container` |
| `flex-direction` | `Inner direction` |
| `gap` | `Inner / Stack / Grid / Split` |
| `align-items` | `Inner align` |
| `justify-content` | `Inner justify` |

### 禁止事項

- Dev Mode の CSS をそのまま page / module.css に貼り付けない
- `Container` に `gap` や `flex-direction` の variant を増やしすぎない
- Figma 側の layer 名に合わせるためだけに DOM を増やさない
- Figma の desktop 幅 `width: 1440px` を実装に持ち込まない

### 許容事項

- Figma 上は `Container` 内包、コード上は `Container + Inner` に分解する
- Figma の token 名とコードの props 名が一致しなくてもよい
- ただし、値と概念は必ず一致させる

---

## 10. Layout Component API

### Section

`Section` はセクション全体の意味・上下余白・背景を担当する。

```tsx
<Section as="section" spacing="default" tone="default">
  ...
</Section>
```
...
- `spacing`: `default` / `nav` / `sm` / `lg` / `none`
- `tone`: `default` / `muted` / `primary` / `transparent`

### Container

`Container` は横幅と左右 padding のみを担当する。
...
- `size`: `2xl` / `3xl` / `4xl` / `5xl` / `6xl` / `7xl` / `full`
- `gutter`: `default` / `none`
...
```tsx
<Inner direction="column" gap="16" align="start" justify="center">
  ...
</Inner>
```
...
```tsx
<Section as="header" spacing="nav">
  <Container size="7xl">
    <Inner direction="row" align="center" justify="between">
      ...
    </Inner>
  </Container>
</Section>
...
<Section spacing="default">
  <Container size="3xl">
    <Inner direction="column" gap="8" align="start">
      ...
    </Inner>
  </Container>
</Section>
...
## 11. Figma から値を拾うときの確認順

1. 最外殻の `Section` を選択する
2. `padding-top / bottom` と `background` を確認する
3. `Container` を選択する
4. `max-width` と左右 padding を確認する
5. 内部の `Div / Inner` を選択する
6. `direction / gap / align / justify` を確認する
7. desktop と mobile の差分を見る
8. 差分が token で表現できるか確認する

tablet 専用デザインが存在しない場合は、`mobile default + lg desktop` を基本にする。  
中間幅の見え方は、Container と自然な折り返しで吸収する。

---

## 12. 今回の Figma テンプレートから採用する初期値
...
--nav-padding-y-mobile: 14px;
--nav-padding-y-desktop: 16px;
```
...
--container-padding-x-desktop: 24px;
--container-2xl: 672px;
--container-3xl: 768px;
...
この値は Figma Dev Mode で複数 section / navbar / rich text に共通して確認したため、テンプレート初期値として採用する。
